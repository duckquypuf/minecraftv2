...
public:
    ...
    Player(World *world, Gamemode gamemode)
    {
        this->world = world;
        ...
        position = glm::vec3(worldWidth / 2.0f * chunkWidth, gamemode == SPECTATOR ? 150.0f : 200.0f, worldWidth / 2.0f * chunkWidth);
    ...
    void updatePhysics(float dt)
    {
        ...
        else
        {
            ...
            if (!isFloating && velocity.y < -waterSinkSpeed)
...
private:
    ...
    void processSpectatorMovement(GLFWwindow *window, float dt)
    {
        ...
        if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
            inputDir += camera->right;
        if (jump)
            velocity += glm::vec3(0.0f, 1.0f, 0.0f) * spectatorSpeed * dt;
        ...
        glm::vec3 targetVelocity = inputDir * spectatorSpeed;

        velocity.x = glm::mix(velocity.x, targetVelocity.x, 1.0f * dt);
        velocity.z = glm::mix(velocity.z, targetVelocity.z, 1.0f * dt);
    }
    ...
    void processNormalMovement(GLFWwindow *window, float dt)
    {
        ...
        isFloating = glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS;
